...
const mediaItems = [
  { id: 'dubai', type: 'Image', title: 'Dubai', text: 'A photo album entry from Dubai.', image: 'https://images.pexels.com/photos/30554306/pexels-photo-30554306.jpeg', tone: 'from-[#e8d8b3] to-[#fca590]' },
  { id: 'dreamy-place', type: 'Image', title: 'Dreamy place', text: 'A dreamy place from the previous photo album.', image: 'https://static.vecteezy.com/system/resources/thumbnails/041/448/144/small/ai-generated-beautiful-landscape-scenery-nature-professionalgraphy-photo.jpg', tone: 'from-[#fca590] to-[#e8d8b3]' },
  { id: 'ucla', type: 'Image', title: 'UCLA university', text: 'A photo album entry from UCLA university.', image: 'https://images.pexels.com/photos/34304261/pexels-photo-34304261.jpeg', tone: 'from-[#e8d8b3] to-[#fca590]' },
  { id: 'hiking', type: 'Image', title: 'Hiking', text: 'A hiking photo from the previous album.', image: 'https://images.pexels.com/photos/27681035/pexels-photo-27681035.jpeg', tone: 'from-[#fca590] to-[#e8d8b3]' },
  { id: 'mantu', type: 'Image', title: 'Mantu', text: 'A photo album entry featuring mantu.', image: 'https://images.pexels.com/photos/32705658/pexels-photo-32705658.jpeg', tone: 'from-[#e8d8b3] to-[#fca590]' },
  { id: 'basketball', type: 'Image', title: 'Basketball', text: 'A basketball photo from the previous album.', image: 'https://images.pexels.com/photos/27857312/pexels-photo-27857312.jpeg', tone: 'from-[#fca590] to-[#e8d8b3]' },
  { id: 'video', type: 'Video', title: 'Caitlin Clark is my main inspiration', text: 'In my opinion, Caitlin Clark is the best WNBA basketball player. Every move she makes shows how fierce, focused, and strong she is.', image: 'https://img.youtube.com/vi/rBJVrezf6uo/hqdefault.jpg', tone: 'from-[#e8d8b3] to-[#fca590]' },
  { id: 'cadillac', type: 'Image', title: 'Cadillac', text: 'A Cadillac photo from the previous album.', image: 'https://images.pexels.com/photos/36591054/pexels-photo-36591054.jpeg', tone: 'from-[#fca590] to-[#e8d8b3]' },
  { id: 'cute-cat', type: 'Image', title: 'Cute cat', text: 'A cute cat photo from the previous album.', image: 'https://images.pexels.com/photos/36594706/pexels-photo-36594706.jpeg', tone: 'from-[#e8d8b3] to-[#fca590]' },
];
...
function MediaPage() {
  const [selected, setSelected] = useState<typeof mediaItems[number] | null>(null);
    ...
    <div className="mb-8 flex flex-wrap items-center justify-between gap-4 border-b border-[hsl(var(--border))] pb-4"><p className="font-mono text-[11px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]" data-testid="text-media-count">09 pieces / mixed format</p><div className="flex items-center gap-2 text-xs text-[hsl(var(--muted-foreground))]"><span className="h-2 w-2 rounded-full bg-[hsl(var(--primary))]" /> Select any piece to look closer</div></div>
      <div className="grid items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-3">{mediaItems.map((item, index) => <button type="button" key={item.id} onClick={() => setSelected(item)} className="media-card focus-ring group flex h-full flex-col overflow-hidden rounded-[1.3rem] border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-left no-underline" data-testid={`card-media-${item.id}`}><div className={`relative h-64 shrink-0 overflow-hidden bg-gradient-to-br ${item.tone}`}><img className="media-art h-full w-full object-cover mix-blend-multiply opacity-80" src={item.image} alt={item.title} onError={(event) => { event.currentTarget.style.display = 'none'; }} />{item.type === 'Video' && <span className="absolute bottom-4 right-4 flex h-10 w-10 items-center justify-center rounded-full bg-[hsl(var(--background)/.9)] text-[hsl(var(--foreground))]"><Play size={15} fill="currentColor" /></span>}<div className="absolute left-4 top-4"><StatusPill tone={item.type === 'Video' ? 'warm' : 'teal'}>{item.type}</StatusPill></div></div><div className="flex flex-1 flex-col p-5"><div className="mb-3 flex items-center justify-between text-[11px] font-mono text-[hsl(var(--muted-foreground))]"><span>{String(index + 1).padStart(2, '0')}</span><ArrowUpRight className="media-arrow" size={16} /></div><h2 className="font-serif text-2xl leading-tight tracking-[-.035em]" data-testid={`text-media-title-${item.id}`}>{item.title}</h2><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{item.text}</p></div></button>)}</div>
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[hsl(222_29%_18%/.75)] p-4 backdrop-blur-sm" role="dialog" aria-modal="true" data-testid="dialog-media-detail"><button className="absolute inset-0 cursor-default" onClick={() => setSelected(null)} aria-label="close media detail" data-testid="button-close-media-backdrop" /><div className="relative z-10 w-full max-w-3xl overflow-hidden rounded-[1.5rem] bg-[hsl(var(--card))] shadow-[var(--shadow-md)]"><IconButton label="close media detail" onClick={() => setSelected(null)} className="focus-ring absolute right-4 top-4 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-[hsl(var(--background)/.9)]"><X size={18} /></IconButton><div className={`h-72 bg-gradient-to-br ${selected.tone} sm:h-96`}>{selected.type === 'Video' ? <iframe className="h-full w-full" src={videoEmbedUrl} title={selected.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /> : <img className="h-full w-full object-cover mix-blend-multiply opacity-85" src={selected.image} alt={selected.title} onError={(event) => { event.currentTarget.style.display = 'none'; }} />}</div><div className="p-6 sm:p-8"><StatusPill tone="warm">{selected.type}</StatusPill><h2 className="mt-4 font-serif text-4xl tracking-[-.05em]" data-testid="text-selected-media-title">{selected.title}</h2><p className="mt-3 max-w-xl leading-7 text-[hsl(var(--muted-foreground))]" data-testid="text-selected-media-description">{selected.text}</p>{selected.type === 'Video' && <a href="https://youtu.be/rBJVrezf6uo?si=_upmNX4gBKp9t6Pu" target="_blank" rel="noreferrer" className="ink-link focus-ring mt-5 inline-flex items-center gap-2 text-sm font-bold no-underline" data-testid="link-selected-youtube">Watch on YouTube <ExternalLink size={15} /></a>}</div></div></div>}
  </main></PageFrame>;
}
